...
def parse_credentials(netloc):
    username = pa***MASKED*** = None
    if '@' in netloc:
        prefix, netloc = netloc.rsplit('@', 1)
        if ':' not in prefix:
            username = prefix
        else:
            username, pa***MASKED*** = prefix.split(':', 1)
    if username:
        username = unquote(username)
    if pa***MASKED***:
        pa***MASKED*** = unquote(pa***MASKED***)
    return username, pa***MASKED***, netloc
...
class PyPIRCFile(object):
        ...
        if os.path.exists(self.filename):
            ...
            if 'distutils' in sections:
                ...
                result = {
                    ...
                    'pa***MASKED***': config.get(server, 'pa***MASKED***'),
    ...
    def update(self, username, pa***MASKED***):
        # import pdb; pdb.set_trace()
        config = configparser.RawConfigParser()
        fn = self.filename
        config.read(fn)
        ...
        config.set('pypi', 'pa***MASKED***', pa***MASKED***)
        with open(fn, 'w') as f:
            config.write(f)
...
def _store_pypirc(index):
    PyPIRCFile().update(index.username, index.pa***MASKED***)
