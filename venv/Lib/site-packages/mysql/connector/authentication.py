...
class BaseAuthPlugin(object):
    ...
    prepare_pa***MASKED***(). When instantiating, auth_data argument is
    required. The username, pa***MASKED*** and database are optional. The
    ssl_enabled argument can be used to tell the plugin whether SSL is
    active or not.

    The method auth_response() method is used to retrieve the pa***MASKED***
    which was prepared by prepare_pa***MASKED***().
    """

    requires_ssl = False
    plugin_name = ''

    def __init__(self, auth_data, username=None, pa***MASKED***=None, database=None,
                 ssl_enabled=False):
        """Initialization"""
        self._auth_data = auth_data
        self._username = username
        self._pa***MASKED*** = pa***MASKED***
        self._database = database
        self._ssl_enabled = ssl_enabled

    def prepare_pa***MASKED***(self):
        """Prepares and returns pa***MASKED*** to be send to MySQL
    ...
    def auth_response(self):
        """Returns the prepared pa***MASKED*** to send to MySQL
...
class MySQLNativePasswordAuthPlugin(BaseAuthPlugin):
    ...
    requires_ssl = False
    plugin_name = 'mysql_native_pa***MASKED***'

    def prepare_pa***MASKED***(self):
        """Prepares and returns pa***MASKED*** as native MySQL 4.1+ pa***MASKED***"""
        if not self._auth_data:
            raise errors.InterfaceError("Missing authentication data (seed)")

        if not self._pa***MASKED***:
            return b''
        pa***MASKED*** = self._pa***MASKED***

        if isstr(self._pa***MASKED***):
            pa***MASKED*** = self._pa***MASKED***.encode('utf-8')
        else:
            pa***MASKED*** = self._pa***MASKED***

        if PY2:
            pa***MASKED*** = buffer(pa***MASKED***)  # pylint: disable=E0602
            try:
        ...
        try:
            ...
            raise errors.InterfaceError(
                "Failed scrambling pa***MASKED***; {0}".format(exc))
...
class MySQLClearPasswordAuthPlugin(BaseAuthPlugin):
    ...
    requires_ssl = True
    plugin_name = 'mysql_clear_pa***MASKED***'

    def prepare_pa***MASKED***(self):
        """Returns pa***MASKED*** as as clear text"""
        if not self._pa***MASKED***:
            return b'\x00'
        pa***MASKED*** = self._pa***MASKED***

        if PY2:
            if isinstance(pa***MASKED***, unicode):  # pylint: disable=E0602
                pa***MASKED*** = pa***MASKED***.encode('utf8')
        elif isinstance(pa***MASKED***, str):
            pa***MASKED*** = pa***MASKED***.encode('utf8')

        return pa***MASKED*** + b'\x00'
