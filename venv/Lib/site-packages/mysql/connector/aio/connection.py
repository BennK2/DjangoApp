...
class MySQLConnection(MySQLConnectionAbstract):
    ...
    async def connect(self) -> None:
        ...
        if (
            ...
            # that defines its use, and the number of se***MASKED*** bits being used.
    ...
    async def _do_handshake(self) -> None:
        ...
        if not self._handshake["capabilities"] & ClientFlag.SSL:
            if not self.is_secure:
                if self._auth_plugin == "mysql_clear_pa***MASKED***":
                    raise InterfaceError(
                        "Clear pa***MASKED*** authentication is not supported over "
                        "insecure channels"
                    )
    ...
    async def _do_auth(self) -> None:
        ...
        Raises NotSupportedError when we get the old, insecure pa***MASKED***
        ...
        pa***MASKED*** = (
            self._pa***MASKED***1
            if self._pa***MASKED***1 and self._pa***MASKED*** != self._pa***MASKED***1
            else self._pa***MASKED***
        )
        ...
        ok_pkt = await self._authenticator.authenticate(
            ...
            username=self._user,
            pa***MASKED***1=pa***MASKED***,
            pa***MASKED***2=self._pa***MASKED***2,
            pa***MASKED***3=self._pa***MASKED***3,
    ...
    async def cmd_change_user(
        ...
        self._pa***MASKED***1 = pa***MASKED***1
        self._pa***MASKED***2 = pa***MASKED***2
        self._pa***MASKED***3 = pa***MASKED***3

        if self._pa***MASKED***1 and pa***MASKED*** != self._pa***MASKED***1:
            self._pa***MASKED*** = self._pa***MASKED***1
