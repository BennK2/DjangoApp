...
class MySQLLdapSaslPasswordAuthPlugin(MySQLAuthPlugin):
    ...
    SCRAM-SHA-1 amd SCRAM-SHA-256
        ...
        The first message from client will be generated by prepare_pa***MASKED***(),
    ...
    def _hmac(self, pa***MASKED***: bytes, salt: bytes) -> bytes:
        digest_maker = hmac.new(pa***MASKED***, salt, self.def_digest_mode)
        return digest_maker.digest()

    def _hi(self, pa***MASKED***: str, salt: bytes, count: int) -> bytes:
        """Prepares Hi
        Hi(pa***MASKED***, salt, iterations) where Hi(p,s,i) is defined as
    ...
    def _first_message_krb(self) -> Optional[bytes]:
        ...
        try:
            ...
            logger.debug(
                "# Stored credentials found, if pa***MASKED*** was given it will be ignored."
            )
            try:
            ...
            try:
                logger.debug("# Attempt to retrieve credentials with given pa***MASKED***")
                acquire_cred_result = gssapi.raw.acquire_cred_with_pa***MASKED***(
                    user_name,
                    self._pa***MASKED***.encode("utf8"),
                    usage="initiate",
                )
                cred = acquire_cred_result[0]
            except gssapi.raw.misc.GSSError as err2:
                raise ProgrammingError(
                    f"Unable to retrieve credentials with the given pa***MASKED***: {err2}"
                ) from err
    ...
    def _second_message(self) -> bytes:
        ...
        where:
            ...
            <client_key>: hmac(salted_pa***MASKED***, b"Client Key")
        ...
        passw = self._normalize(self._pa***MASKED***)
        salted_pa***MASKED*** = self._hi(passw, b64decode(self.server_salt), self.iterations)
        logger.debug("salted_pa***MASKED***: %s", b64encode(salted_pa***MASKED***).decode())

        client_key = self._hmac(salted_pa***MASKED***, b"Client Key")
        ...
        server_key = self._hmac(salted_pa***MASKED***, b"Server Key")
    ...
    def _validate_second_reponse(self, servers_second: bytearray) -> bool:
        ...
        The second message from the server consist of the server's proof:
            ...
            where:
                <server_key>: hmac(<salted_pa***MASKED***>, b"Server Key")
                <auth_msg>: <client_first_no_header>,<servers_first>,
                            c=<client_header>,r=<server_nonce>
