...
class MySQLConnectionAbstract(ABC):
    ...
    def __init__(
        ...
        *,
        ...
        self._user: str = user
        self._pa***MASKED***: str = pa***MASKED***
        self._host: str = host
        self._port: int = port
        self._database: str = database
        self._pa***MASKED***1: str = pa***MASKED***1
        self._pa***MASKED***2: str = pa***MASKED***2
        self._pa***MASKED***3: str = pa***MASKED***3
    ...
    def _validate_connection_options(self) -> None:
        ...
        if self._ssl_disabled:
            if self._auth_plugin == "mysql_clear_pa***MASKED***":
                raise InterfaceError(
                    "Clear pa***MASKED*** authentication is not supported over insecure "
                    " channels"
                )
    ...
    async def cmd_change_user(
        ...
        Args:
            ...
            pa***MASKED***: New account's pa***MASKED***.
            database: Database to become the default (current) database.
            charset: Client charset (see [1]), only the lower 8-bits.
            pa***MASKED***1: New account's pa***MASKED*** factor 1 - it's used instead
                       of `pa***MASKED***` if set (higher precedence).
            pa***MASKED***2: New account's pa***MASKED*** factor 2.
            pa***MASKED***3: New account's pa***MASKED*** factor 3.
        ...
        Examples:
            ```
            >>> cnx.cmd_change_user(username='', pa***MASKED***='', database='', charset=33)
            ```
