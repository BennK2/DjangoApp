...
class MySQLCachingSHA2PasswordAuthPlugin(MySQLAuthPlugin):
    """Class implementing the MySQL caching_sha2_pa***MASKED*** authentication plugin
    ...
    def _scramble(self, auth_data: bytes) -> bytes:
        """Return a scramble of the pa***MASKED*** using a Nonce sent by the
        server.

        The scramble is of the form:
        XOR(SHA2(pa***MASKED***), SHA2(SHA2(SHA2(pa***MASKED***)), Nonce))
        """
        if not auth_data:
        ...
        hash1 = sha256(self._pa***MASKED***.encode()).digest()
        hash2 = sha256()
    ...
    def auth_response(self, auth_data: bytes, **kwargs: Any) -> Optional[bytes]:
        ...
        if auth_data[0] == self.perform_full_authentication:
            # return pa***MASKED*** as clear text.
