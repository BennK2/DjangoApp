...
class MySQLKerberosAuthPlugin(MySQLBaseKerberosAuthPlugin):
    ...
    def _acquire_cred_with_pa***MASKED***(self, upn: str) -> gssapi.raw.creds.Creds:
        """Acquire and store credentials through provided pa***MASKED***.
        ...
        """
        logger.debug("Attempt to acquire credentials through provided pa***MASKED***")
        ...
        try:
            ...
            raise ProgrammingError(
                f"Unable to acquire credentials with the given pa***MASKED***: {err}"
            ) from err
        return creds
    ...
    def auth_response(
        ...
        if spn is None:
            return self._pa***MASKED***.encode() + b"\x00"
        ...
        try:
            ...
            if self._username and self._username != creds_user:
                ...
                if self._pa***MASKED*** is not None:
                    creds = self._acquire_cred_with_pa***MASKED***(upn)
            if creds_realm and creds_realm != realm and self._pa***MASKED*** is not None:
                creds = self._acquire_cred_with_pa***MASKED***(upn)
        except gssapi.raw.exceptions.ExpiredCredentialsError as err:
            if upn and self._pa***MASKED*** is not None:
                creds = self._acquire_cred_with_pa***MASKED***(upn)
            else:
...
class MySQLSSPIKerberosAuthPlugin(MySQLBaseKerberosAuthPlugin):
    ...
    def auth_response(
        ...
        if self._username and self._pa***MASKED***:
            _auth_info = (self._username, realm, self._pa***MASKED***)
        else:
            _auth_info = None
