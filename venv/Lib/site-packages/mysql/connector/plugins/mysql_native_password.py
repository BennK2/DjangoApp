...
class MySQLNativePasswordAuthPlugin(MySQLAuthPlugin):
    ...
    def _prepare_pa***MASKED***(self, auth_data: bytes) -> bytes:
        ...
        hash4 = None
        try:
            hash1 = sha1(self._pa***MASKED***.encode()).digest()
            ...
            raise InterfaceError(f"Failed scrambling pa***MASKED***; {err}") from err
    ...
    def auth_response(self, auth_data: bytes, **kwargs: Any) -> Optional[bytes]:
        ...
        """
        return self._prepare_pa***MASKED***(auth_data)
