...
class MySQLConnectionAbstract(object):
    ...
    def config(self, **kwargs):
        ...
        if 'user' in config or 'pa***MASKED***' in config:
            try:
                user = config['user']
                del config['user']
            except KeyError:
                user = self._user
            try:
                pa***MASKED*** = config['pa***MASKED***']
                del config['pa***MASKED***']
            except KeyError:
                pa***MASKED*** = self._pa***MASKED***
            self.set_login(user, pa***MASKED***)
    ...
    def set_login(self, username=None, pa***MASKED***=None):
        """Set login information for MySQL

        Set the username and/or pa***MASKED*** for the user connecting to
        the MySQL Server.
        """
